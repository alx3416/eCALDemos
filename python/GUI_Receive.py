...
class App:
    def __init__(self, window, window_title, video_source=0):
        ...
        self.sub = ProtoSubscriber("webcam_data", mensaje_main_pb2.webcam)
        self.protobuf_message = mensaje_main_pb2.webcam()
        self.is_sub_receiving = False
        ...
        self.chkbox_ecal = tkinter.Checkbutton(window, text='eCAL Receive', variable=self.checkbox_ecal)
    ...
    def snapshot(self):
        ...
        ret, self.protobuf_message, time = self.sub.receive(1)
        buffer = np.frombuffer(self.protobuf_message.frame.data,
                               dtype=np.uint8)
    ...
    def update(self):
        # If activated, receive ecal message
        ret, msg, time = self.sub.receive(10)
        if ret:
            self.is_sub_receiving = True
            self.protobuf_message = msg
        if self.checkbox_ecal.get() and self.is_sub_receiving:
